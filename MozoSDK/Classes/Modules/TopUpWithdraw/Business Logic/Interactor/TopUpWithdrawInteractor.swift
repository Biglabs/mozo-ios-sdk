...
import Foundation
class TopUpWithdrawInteractor: NSObject {
    var output: TopUpWithdrawInteractorOutput?
    ...
    @objc func onUserDidCloseAllMozoUI(_ notification: Notification) {
        print("TopUpWithdrawInteractor - User close Mozo UI, clear pin cache")
        pinToRetry = nil
    }
    ...
    func sendPrepareWithdrawTransaction(_ eventId: Int64) {
        _ = apiManager?.withdrawTopUp().done { (interTx) in
...
extension TopUpWithdrawInteractor: TopUpWithdrawInteractorInput {
    func withdraw() {
        _ = apiManager?.withdrawTopUp().done { (interTx) in
            self.transaction = interTx
            self.requestForPin()
    ...
    func sendSignedTx(pin: String) {
        signManager?.signTransaction(transaction!, pin: pin)
            .done { (signedInterTx) in
                self.apiManager?.sendSignedWithdrawTopUpTx(signedInterTx).done({ (receivedTx) in
