...
public class TxHistoryDisplayCollection {
    ...
    func displayItemForTxHistoryDTO(_ txHistory: TxHistoryDTO) -> TxHistoryDisplayItem {
        ...
        let fromNameWithDate = self.buildFromNameWithDate(addressFrom: txHistory.addressFrom ?? "", addressTo: txHistory.addressTo ?? "", action: action, date: date, topUpReason: topUpReasonEnum)
        return TxHistoryDisplayItem(action: action, date: date, fromNameWithDate: fromNameWithDate, amount: amount, exAmount: exAmount, txStatus: txHistory.txStatus ?? "", addressFrom: txHistory.addressFrom, addressTo: txHistory.addressTo, topUpReason: topUpReasonEnum)
    }
