...
extension ApiManager {
    ...
    func syncAddressFromBranchIntoBeacon(_ beaconId: Int64) -> Promise<BeaconInfoDTO> {
        return Promise { seal in
            let url = Configuration.BASE_STORE_URL + RETAILER_BEACON_API_PATH + "/syncAddressFromBranchIntoBeacon/\(beaconId)"
            self.execute(.put, url: url)
                .done { json -> Void in
                    // JSON info
                    print("Finish request to sync address from branch into beacon by id [\(beaconId)], json response: \(json)")
                    let jobj = JSON(json)
                    if let beaconInfo = BeaconInfoDTO(json: jobj) {
                        seal.fulfill(beaconInfo)
                    } else {
                ...
                .catch { error in
                    print("Error when request sync address from branch into beacon by id [\(beaconId)]: " + error.localizedDescription)
                    //Handle error or give feedback to the user
                    seal.reject(error)
                }
                .finally {
    ...
    func syncLocationFromBranchIntoBeacon(_ beaconId: Int64) -> Promise<BeaconInfoDTO> {
        return Promise { seal in
            let url = Configuration.BASE_STORE_URL + RETAILER_BEACON_API_PATH + "/syncLocationFromBranchIntoBeacon/\(beaconId)"
            self.execute(.put, url: url)
                .done { json -> Void in
                    // JSON info
                    print("Finish request to sync location from branch into beacon by id [\(beaconId)], json response: \(json)")
                    let jobj = JSON(json)
                    if let beaconInfo = BeaconInfoDTO(json: jobj) {
                        seal.fulfill(beaconInfo)
                    } else {
                ...
                .catch { error in
                    print("Error when request sync location from branch into beacon by id [\(beaconId)]: " + error.localizedDescription)
                    //Handle error or give feedback to the user
                    seal.reject(error)
                }
                .finally {
