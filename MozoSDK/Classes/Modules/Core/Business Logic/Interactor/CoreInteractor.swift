...
class CoreInteractor: NSObject {
    ...
    private func getUserProfile() -> Promise<Void> {
        return Promise { seal in
            _ = apiManager.getUserProfile().done { (userProfile) in
                if let userId = userProfile.userId, !userId.isEmpty {
    ...
    func checkWallet(module: Module) {
        ...
        // Check wallet
        let user = SessionStoreManager.loadCurrentUser()
        if let wallet = user?.profile?.walletInfo, wallet.encryptSeedPhrase != nil, let id = user?.id {
